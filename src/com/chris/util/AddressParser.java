...
import com.chris.test.AddressTest;
...
public class AddressParser {
	...
	public static void main(String[] args) {
		...
		AddressParser t = new AddressParser();
//		t.parseFile();
//		t.parseString();
//		t.parseAddressLiFile();
		ArrayList<Address> addList = t.parseAddressByURL("http://suzhou.anjuke.com/community/props/sale/171651", "北京", "大兴");
		if(addList==null)
			System.out.println("is null");
		else
			System.out.println(addList.size());
		
//		System.out.println(getCityInPage("http://guangzhou.anjuke.com/rental/hp36-p8"));
//		System.out.println(getDistrictInPage("http://guangzhou.anjuke.com/rental/hp36-p8"));
...
//			System.out.println(codes.html());
	...
	public static ArrayList<Address> parseAddressByURL(String url, String city, String district) {
		...
		try {
			...
			for(int i=1; i < 26; i++){
				...
				if(addstr == null || addstr.equals(""))return null;
	...
	public static void storeStart2EndIndex(String url, int start, int end, String city, String district){
		for(int i=start; i<=end; i++){
			...
			ArrayList<Address> addList = AddressParser.parseAddressByURL("http://"+ city + ".anjuke.com/sale/"+district+"/p"+ i +"/", "北京", "大兴");
	...
	public static String getCityInPage(String url){
		//url = "http://beijing.anjuke.com/sale/daxing/p1/";
		Document doc;
		try {
			doc = Jsoup.connect(url).get();
			Elements codes = doc.body().select("span[class~=^city$]");
			if(codes!=null && codes.size()!=0){
				String city = codes.get(0).html();
				return city;
			}else{
				return null;
			}
	...
	public static String getDistrictInPage(String url){
		//url = "http://beijing.anjuke.com/tycoon/";
		Document doc;
		try {
			doc = Jsoup.connect(url).get();
			Elements codes = doc.body().select("div[class~=^condition_links2 condition_link2_def$]");
			if(codes!=null && codes.size()!=0){
				Element elem = codes.select("div").get(0);
				Elements elems = elem.select("strong");
				String district = elems.get(0).html();
				return district;
			}else{
				return null;
			}
